...
BaseType_t xPortStartScheduler( void )
{
	...
	/* 启动第一个任务，从软件中断中进行任务调度, 程序出该函数就不在返回继续执行了 */
	prvStartFirstTask();
...
__asm void prvStartFirstTask( void )
{
	...
	ldr r0, =0xE000ED08  // 该地址是向量表偏移量寄存器，大多数stm32芯片的向量表开始地址定义到了 0X08000000。
	ldr r0, [r0]
	ldr r0, [r0]
    ...
    /* 调用SVC去启动第一个任务 */
	svc 0  /*  触发一个 `0` 号 软件中断（SuperVisor Call），以启动第一个任务。在FreeRTOS中，这通常用于切换到第一个任务的上下文。 */
	nop
	nop
}
